...
* V1:
    ...
    - If the User chooses, he could append the URL, Authentication and Replacement Map in a `before callback`.
    - He can delete them in a `after callback`.
    - This is done so the User can have more control over the REST call (a lot of APIs require Pagination using Request
      Params).
* V2:
    - The User does not provide us with sensitive information like URL or Authentication.
    - Instead, he provides us with a Callbacks that return Promises that Resolve in Replacement Maps.
    - Note: This is not the `Search Replacement Map` from V1. It holds the information we directly replace in the `div`.
