...
const authEventHandler = {
  onSuccessfulLoginHandler: defaultOnSuccessfulHandler,
  onErrorHandler: () => {
...
function setUpLoginEvent() {
  ...
  loginForm.addEventListener('submit', function (event) {
    ...
    login(email, password)
      .then((data) => {
        authEventHandler.onSuccessfulLoginHandler(data);
        addUserOnline(email);
      })
      .catch((errorHandler) => {
        errorHandler();
...
function setResponseEvents(onSuccessfulLoginCb, onErrorCb, onServerError) {
  if (onSuccessfulLoginCb !== undefined) {
    authEventHandler.onSuccessfulLoginHandler = onSuccessfulLoginCb;
  }
  if (onErrorCb !== undefined) {
    authEventHandler.onErrorHandler = onErrorCb;
  }
  if (onServerError !== undefined) {
    authEventHandler.onServerErrorHandler = onServerError;
  }
}
...
function defaultOnSuccessfulHandler(data) {
